...
import 'package:receiptscanner/logic/pdf_generator.dart';
...
// --- 編集・詳細・全画面表示 統合画面 ---
// Stackレイアウト: 背景に全画面画像、手前にBottom Sheetフォーム
...
// 背景用の画像をロード
    ...
    if (path != null && path.toLowerCase().endsWith('.pdf')) {
      try {
        ...
        if (await file.exists()) {
          ...
          // 【修正】scaleパラメータを削除しました（デフォルト解像度で読み込みます）
  ...
  void dispose() {
    ...
    _timeController.dispose(); _invoiceController.dispose(); _transformationController.dispose(); super.dispose();
  }
  ...
  // UI構築: Stack (背景画像 + DraggableSheet)
  @override
  Widget build(BuildContext context) {
    final colorScheme = Theme.of(context).colorScheme;
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // 1. 最背面: 拡大縮小可能な画像ビューアー
          Positioned.fill(
            // 【修正】InteractiveViewer全体をGestureDetectorでラップする
            // これにより、画像がない空白部分をタップしても反応するようになる
            child: GestureDetector(
              onTap: () {
                _transformationController.value = Matrix4.identity();
              },
              // タッチイベントを確実に拾うために透明色を指定するなどは不要（デフォルトで透過）
              // ただしInteractiveViewerがタッチを消費しない場合にここに来る
              child: InteractiveViewer(
                transformationController: _transformationController,
                minScale: 0.1,
                maxScale: 10.0,
                boundaryMargin: const EdgeInsets.all(double.infinity),
                // 【修正】内部のGestureDetectorは削除（親で検知するため）
                child: Container(
                  color: Colors.black,
          ...
          // 2. 前面: 編集フォーム (DraggableScrollableSheet)
